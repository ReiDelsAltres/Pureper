...
export default class IfRule extends SyntaxRule {
    name = 'if';
    priority = 20; // Выполняется раньше @expression
    find(template) {
        const results = [];
    ...
    parseIfChain(template, startIdx) {
        const items = [];
        let pos = startIdx;
        // Parse @if
    ...
    parseConditionBlock(template, start, keyword) {
...
//# sourceMappingURL=IfRule.js.map